...
export class ClientMiddleware {
  ...
  async use(@Req() req: Req, @Res() res: Res, @Context() ctx: Context) {
    const clientId = (req.headers["client-id"] as string) || (req.body["client_id"] as string);
    const clientSecret = (req.headers["client-secret"] as string) || (req.body["client_secret"] as string);
